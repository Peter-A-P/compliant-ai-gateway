...
| `boundary ledger report` | 0.1 | Calls, tokens and cost by month, environment, project, declared data class (0.4) and model. `--data-class personal` (0.4) keeps only the rows whose caller declared that class; `--data-class undeclared` keeps the rows that declared none. A misspelt class exits 2 rather than printing an empty table |
