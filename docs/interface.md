...
| Sweep | `sweep(pages, spans, *, first_page=1, vocabulary=DECISION_VOCABULARY, allow=()) -> list[Span]` | 0.5.6 | A person found anywhere licenses the other whole-word occurrences of their name parts everywhere. Returns the given spans plus the found ones, overlaps resolved; the added ones carry `recogniser == SWEEP_ID` (`boundary:sweep`). Case-sensitive, whole word, token runs not single words, vocabulary words refused. 07 measured +4.6 points of person recall on its hard name pool |
