...
| 6 | **Tag `v0.2.0`**, once items 3 and 4 have each been exercised live. Item 4 is done. Item 3 needs the `smoke` workflow added here on 2026-09-11 to be run with `--batch`, which needs the four vendor keys as repository secrets on this repository; they exist today only on the release-gate repository. Then CHANGELOG dated, the `since` column in `docs/interface.md` checked, version `0.2.0`, 02 pins `>=0.2,<1`. No need to wait for Sep 27: 03 pins the `v0.1.0` tag, so nothing on `main` can reach it | As soon as the batch call is made | Tagged; 02 installs from the tag with the same read-only token 03 uses |
