...
- [ ] v0.2: Foundry, Bedrock and Vertex exercised once each, calls in the ledger; Anthropic batches; `ledger merge` and `report`. **`merge` and `report` done 2026-09-10. Anthropic batches done 2026-09-11. Bedrock exercised live from GitHub Actions 2026-09-16**: smoke run #3, `ca-central-1`, status 200, 14 in and 4 out, 943 ms, row 5 of the run's ledger, `region = ca-central-1` and `residency = geo` recorded, `costed = 0` because AWS publishes its own rates and they are not in the price files. **Vertex reached live 2026-09-16 and was refused**: smoke #4, project `gen-lang-client-0915051085`, global endpoint, HTTP 429 `Quota exceeded for global_online_prediction_requests_per_base_model` on the first request the account ever made, retried three times per policy and refused each time. The ledger row exists with `error_type = http_429` and `retries = 3`, which is the rule working: every call writes a row including failures. **So all three platforms now refuse or ration Claude to a new account**, and the adapter is not what is missing on either of the two outstanding ones. Foundry needs a quota grant, Vertex needs a quota grant
