...
| 9 | **Bedrock adapter.** ~~`InvokeModel` ... SigV4 through the botocore signer~~ **Superseded 2026-09-15:** the Anthropic Messages route with an API key, so a subclass of the Anthropic adapter with no new dependency. Adapter, residency guard and 31 tests landed 2026-09-15; the account opened early, and the AWS CLI confirmed model access and a live call from `ca-central-1` | ~~After the AWS account opens (first week of October)~~ Done, apart from the row itself. The `smoke` workflow has a bedrock step, skipped until an `AWS_BEARER_TOKEN_BEDROCK` repository secret exists, which is Peter's action and the only thing between here and an uncosted live row | `v0.2.2` tagged; a **costed** row in the ledger. Blocked on prices: AWS publishes its own rates and they were not readable on 2026-09-15, so a Bedrock call is uncosted until they are copied in |
